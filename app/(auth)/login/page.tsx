...
function LoginInner() {
  ...
  const [loading, setLoading] = useState<null | "password" | "magic" | "google">(null)
  ...
  async function handleGoogle() {
    ...
    // On success, the user is redirected away, so we don't reset loading.
  }

  return (
    <div className="glass-card rounded-2xl p-8">
      <h1 className="font-display text-2xl font-semibold text-foreground mb-1">Sign in</h1>
      <p className="text-sm text-muted-foreground mb-6">Welcome back. Pick how you want to get in.</p>
      ...
      <form onSubmit={handlePasswordLogin} className="space-y-3">
        ...
        <button
          ...
          style={{
            ...
            boxShadow:
              "0 4px 14px rgba(160,100,0,0.30), inset 0 1px 0 rgba(255,255,255,0.22)",
          }}
        >
          {loading === "password" ? <Loader2 size={16} className="animate-spin" /> : "Sign in"}
        </button>
      </form>
      ...
      <button
        ...
        disabled={loading !== null}
        className="w-full mt-3 text-sm font-medium text-foreground hover:text-[#A8843A] transition py-2 disabled:opacity-50"
      >
        {loading === "magic" ? (
          <span className="inline-flex items-center gap-2 justify-center"><Loader2 size={14} className="animate-spin" /> Sending link...</span>
        ) : (
      ...
      {/* Links */}
      <div className="mt-6 pt-5 border-t border-[rgba(201,168,92,0.18)] text-xs text-muted-foreground flex justify-between">
        <Link href="/forgot-password" className="hover:text-foreground transition">Forgot password?</Link>
        <Link href="/signup" className="hover:text-foreground transition font-medium">Create account →</Link>
      </div>
    </div>
  )
}
